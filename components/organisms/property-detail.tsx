...
export default function PropertyDetail() {
  ...
  useEffect(() => {
    ...
    async function fetchProperty() {
      setIsLoading(true);
      const res = await propertyService.getProperty(currentPropertyId);
      if (res.status === 200) {
    ...
    const fetchImages = async () => {
      const res = await propertyService.getPropertyImages(currentPropertyId);
      if (res.status === 200) {
        setImages(res.data);
    ...
    const fetchFavorites = async () => {
      ...
      if (res.status === 200) {
        const favorite = res.data.some(
          (prop: Favorite) =>
            String(prop.property_id) === String(currentPropertyId) &&
            String(user?.id) === String(prop.user_id),
        );
        if (favorite) {
          setIsFavorite(true);
        }
