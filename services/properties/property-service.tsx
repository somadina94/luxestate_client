...
function validRouteId(id: number | string): boolean {
  if (typeof id === "number") return Number.isFinite(id);
  return id.trim().length > 0 && id !== "NaN";
}
...
class PropertyService {
  ...
  async uploadPhoto(
    ...
    try {
      if (!validRouteId(propertyId)) {
        return { status: 400, data: null, message: "Invalid property id" };
      }
  ...
  async getPropertyImages(propertyId: number | string) {
    try {
      if (!validRouteId(propertyId)) {
        return { status: 400, data: [], message: "Invalid property id" };
      }
  ...
  async updateImageOrder(
    ...
    try {
      if (!validRouteId(imageId)) {
        return { status: 400, data: null, message: "Invalid image id" };
      }
      const response = await axiosInstance.patch(
  ...
  async markImagePrimary(imageId: number | string, access_token: string) {
    try {
      if (!validRouteId(imageId)) {
        return { status: 400, data: null, message: "Invalid image id" };
      }
  ...
  async deleteImage(imageId: number | string, access_token: string) {
    try {
      if (!validRouteId(imageId)) {
        return { status: 400, data: null, message: "Invalid image id" };
      }
  ...
  async deleteProperty(propertyId: number | string, access_token: string) {
    try {
  ...
  async getProperty(propertyId: number | string) {
    try {
  ...
  async markAsFavorite(propertyId: number | string, access_token: string) {
  ...
  async removeFromFavorite(propertyId: number | string, access_token: string) {
    try {
